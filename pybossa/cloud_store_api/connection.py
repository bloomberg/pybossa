...
def create_connection(**kwargs):
    ...
    logger.info("=== Boto Connection Creation Start ===")
    ...
    logger.info("AWS V2 Environment Variables:")
    logger.info("  AWS_V2_ACCESS_KEY_ID: %s", v2_access if v2_access else "not_set")
    logger.info("  AWS_V2_SECRET_ACCESS_KEY: %s", _mask_secret(v2_secret) if v2_secret else "not_set")
    logger.info("  AWS_V2_REGION: %s", v2_region)
    ...
    logger.info("Standard AWS Environment Variables:")
    logger.info("  AWS_ACCESS_KEY_ID: %s", std_access if std_access else "not_set")
    logger.info("  AWS_SECRET_ACCESS_KEY: %s", _mask_secret(std_secret) if std_secret else "not_set")
    logger.info("  AWS_DEFAULT_REGION/AWS_REGION: %s", std_region)
    ...
    logger.info("Connection kwargs provided:")
    ...
    logger.info("Access Key to be used: %s", access_key)

    # Log connection settings
    endpoint = kwargs.get("endpoint", kwargs.get("host", "default_s3_endpoint"))
    logger.info("Connection Settings:")
    logger.info("  Endpoint/Host: %s", endpoint)
    logger.info("  Region: %s", kwargs.get("region", std_region))
    logger.info("  SSL Enabled: %s", kwargs.get("is_secure", True))
    logger.info("  SSL Verify: %s", not kwargs.get("s3_ssl_no_verify", False))
    logger.info("  Proxy URL: %s", kwargs.get("proxy_url", "not_set"))

    store = kwargs.pop("store", None)
    check_store(store)
    store_type_v2 = current_app.config.get("S3_CONN_TYPE_V2")

    logger.info("Store type: %s (V2 type: %s)", store, store_type_v2)

    if store and store == store_type_v2:
        logger.info("Creating CustomConnectionV2 (boto3)")
    ...
    if 'object_service' in kwargs:
        logger.info("Creating ProxiedConnection (boto2)")
        conn = ProxiedConnection(**kwargs)
    else:
        logger.info("Creating CustomConnection (boto2)")
        conn = CustomConnection(**kwargs)

    logger.info("=== Boto Connection Creation Complete ===")
    return conn
...
class CustomConnection(S3Connection):
    ...
    def __init__(self, *args, **kwargs):
        logger.info("CustomConnection.__init__ called")
        logger.info("  args count: %d", len(args))
        ...
        logger.info("  Provider configured: aws_access_key_id=%s", kwargs['provider'].access_key)
        ...
        logger.info("  SSL no verify: %s", ssl_no_verify)
        logger.info("  Host suffix: %s", self.host_suffix)
        ...
        logger.info("CustomConnection initialized: host=%s, port=%s", self.host, self.port)
...
class CustomConnectionV2(BaseConnection):
    def __init__(
        ...
        logger.info("CustomConnectionV2.__init__ called (boto3)")
        logger.info("  aws_access_key_id: %s", aws_access_key_id)
        logger.info("  aws_secret_access_key: %s", _mask_secret(aws_secret_access_key))
        logger.info("  endpoint: %s", endpoint)
        logger.info("  cert (SSL verify): %s", cert)
        logger.info("  proxy_url: %s", proxy_url)
        ...
        if proxy_url:
            ...
            logger.info("  Proxy configuration set: %s", config_kwargs['proxies'])
        ...
        logger.info("CustomConnectionV2 boto3 s3 client created successfully")
        logger.info("  Client endpoint: %s", self.client.meta.endpoint_url)
        logger.info("  Client region: %s", self.client.meta.region_name)
...
class ProxiedConnection(CustomConnection):
    ...
    def __init__(self, client_id, client_secret, object_service, *args, **kwargs):
        logger.info("ProxiedConnection.__init__ called")
        logger.info("  client_id: %s", client_id)
        logger.info("  client_secret: %s", _mask_secret(client_secret))
        logger.info("  object_service: %s", object_service)
        ...
        logger.info("ProxiedConnection initialized")
    ...
    def make_request(self, method, bucket='', key='', headers=None, data='',
        ...
        logger.info("ProxiedConnection.make_request called")
        logger.info("  method: %s", method)
        logger.info("  bucket: %s", bucket)
        logger.info("  key: %s", key)
        logger.info("  x-objectservice-id: %s", headers['x-objectservice-id'])
        logger.info("  jwt token length: %d", len(headers['jwt']))
    ...
    def create_jwt(self, method, host, bucket, key):
        ...
        logger.info("ProxiedConnection.create_jwt called")
        logger.info("  method: %s", method)
        logger.info("  host: %s", host)
        logger.info("  bucket: %s", bucket)
        logger.info("  key: %s", key)
        logger.info("  path: %s", path)
        logger.info("  issuer (client_id): %s", self.client_id)
